  ...
  return (
    <>
      ...
      <motion.div 
        ...
        className="fixed right-0 top-0 h-full w-96 bg-gradient-to-b from-white to-gray-50 shadow-2xl z-50 flex flex-col border-l border-gray-200"
      >
        {/* Header */}
        ...
        {savings > 0 && (
          <motion.div
            ...
            className="bg-gradient-to-r from-green-500 to-emerald-500 text-white p-3 mx-4 mt-4 rounded-lg flex items-center space-x-2"
          >
            <Gift className="w-5 h-5" />
            <span className="text-sm font-medium">You're saving ${savings.toFixed(2)}!</span>
          </motion.div>
        )}
              ...
              <div className="space-y-4">
                {cartItems.map((item, index) => (
                  <motion.div
                    ...
                    className="flex items-center space-x-4 p-4 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow"
                  >
                    <div className="relative">
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-sm text-gray-800 truncate">{item.products.name}</h3>
                      <div className="flex items-center space-x-2 mt-1">
                        <p className="text-blue-600 font-bold">${item.products.price}</p>
                        {item.products.original_price && item.products.original_price > item.products.price && (
                          <p className="text-gray-400 line-through text-sm">${item.products.original_price}</p>
                        )}
                      </div>
