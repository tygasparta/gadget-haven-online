...
const MobileProductCard: React.FC<MobileProductCardProps> = ({
  id, name, price, originalPrice, rating, reviews, image, discount, isFlash, countdownTimer
}) => {
  ...
  const handleWishlist = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!user) { navigate('/auth'); return; }
    toast({ title: "Added to wishlist", description: `${name} has been added to your wishlist.` });
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] relative">
      ...
      <div className="p-4">
        <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2 text-sm leading-tight cursor-pointer hover:text-sky-600 transition-colors" onClick={handleProductClick}>
          {name}
        </h3>
