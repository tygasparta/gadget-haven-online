...
const Header = () => {
  ...
  const searchResults = products.filter(product => product.name.toLowerCase().includes(searchTerm.toLowerCase()) || product.category?.toLowerCase().includes(searchTerm.toLowerCase()) || product.brand?.toLowerCase().includes(searchTerm.toLowerCase())).slice(0, 5); // Limit to 5 results
  ...
  return <>
      ...
      <header className="bg-white shadow-sm border-b sticky top-0 z-40">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 max-w-2xl mx-4 lg:mx-8 hidden md:block relative">
              <form onSubmit={handleSearch} className="relative">
                <Input type="text" placeholder="Search for smartphones, electronics, gadgets..." className="w-full pl-4 pr-12 py-3 border-2 border-gray-200 rounded-lg focus:border-blue-500" value={searchTerm} onChange={handleSearchInputChange} onFocus={() => searchTerm.length > 2 && setShowSearchResults(true)} onBlur={() => setTimeout(() => setShowSearchResults(false), 200)} />
              ...
              {showSearchResults && searchResults.length > 0 && <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg mt-1 shadow-lg z-50 max-h-80 overflow-y-auto">
                  {searchResults.map(product => <div key={product.id} className="flex items-center p-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0" onClick={() => {
                navigate(`/product/${product.id}`);
                setShowSearchResults(false);
                setSearchTerm('');
              }}>
                      <img src={product.image} alt={product.name} className="w-12 h-12 object-cover rounded-lg mr-3" onError={e => {
                  ...
                  <div className="p-2 border-t border-gray-100">
                    <button onClick={handleSearch} className="w-full text-center text-blue-600 hover:text-blue-700 font-medium py-2">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-4">
              {user ? <>
                  ...
                  {isAdmin && <Button onClick={handleAdminClick} className="p-2 hidden sm:flex bg-gradient-to-r from-red-500 to-pink-500 text-white hover:from-red-600 hover:to-pink-600 border-0 shadow-lg animate-pulse hover:animate-none">
                  ...
                  <Button variant="ghost" className="p-2 relative" onClick={handleCartClick}>
                    ...
                    {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-blue-500 text-white rounded-full w-4 h-4 sm:w-5 sm:h-5 text-xs flex items-center justify-center">
                        {cartCount}
                      </span>}
                  </Button>
              ...
              <Button variant="ghost" className={`p-2 md:hidden relative transition-all duration-300 ${isMobileMenuOpen ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg scale-110' : 'hover:bg-gray-100'}`} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        {isMobileMenuOpen && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 md:hidden" onClick={closeMobileMenu}>
            <div className="fixed top-0 right-0 h-full w-80 bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 shadow-2xl transform transition-all duration-500 ease-out" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 border-b border-gray-200/50 bg-gradient-to-r from-blue-500 to-purple-600 text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  {/* User Info Section */}
                  {user && <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 mb-4">
                      <div className="flex items-center space-x-3">
              ...
              <div className="flex flex-col p-4 space-y-3 max-h-[calc(100vh-200px)] overflow-y-auto">
                {menuItems.map((item, index) => <Link key={item.path} to={item.path} className={`group flex items-center p-4 rounded-2xl transition-all duration-300 hover:scale-105 transform ${item.bgColor} hover:shadow-lg border border-gray-200/50`} onClick={closeMobileMenu} style={{
              animationDelay: `${index * 100}ms`
                ...
                <div className="pt-4 border-t border-gray-200/50 space-y-3">
                  {user ? <>
                      ...
                      {isAdmin && <Button onClick={handleAdminClick} className="w-full justify-start p-4 h-auto bg-gradient-to-r from-red-500 to-pink-500 text-white hover:from-red-600 hover:to-pink-600 shadow-lg rounded-2xl group border-0">
